    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-bold text-slate-800">Vue d'ensemble - Administration</h2>
                    <p className="text-slate-600 mt-1">Tableau de bord consolidé multi-magasins</p>
                </div>
                        ...
                        <div className="space-y-3">
                            {data.topProducts.slice(0, 10).map((item, index) => (
                                <div key={item.product.id} className="flex items-center justify-between p-2 hover:bg-slate-50 rounded-lg transition">
                                    <div className="flex items-center gap-3">
                                        <span className={`flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${index === 0 ? 'bg-yellow-100 text-yellow-700' :
                                    ...
                                    <div className="text-right">
                                        <p className="font-semibold text-slate-900">{item.quantitySold} unités</p>
                    ...
                    <div className={theme.card.body}>
                        {data.lowStockProducts.length === 0 ? (
                            <div className="text-center py-8">
                                ...
                                <p className="text-slate-600">Tous les stocks sont au niveau optimal !</p>
                            </div>
                        ) : (
                            <div className="space-y-3">
            ...
            <div className={theme.card.base}>
                <div className={theme.card.header}>
                    <div className="flex items-center justify-between">
                        ...
                        <a href="#" className="text-sm text-indigo-600 hover:text-indigo-800">Voir tout →</a>
                    </div>
                </div>
                <div className={theme.card.body}>
                                ...
                                {data.recentSales.map((sale) => (
                                    <tr key={sale.id} className="hover:bg-slate-50 transition">
                                        <td className="py-3 px-4">
                                            <span className="font-mono text-sm text-slate-900">{sale.saleNumber}</span>
                                        </td>
                                        <td className="py-3 px-4 text-sm text-slate-700">{sale.store.name}</td>
                                        <td className="py-3 px-4 text-sm text-slate-700">{sale.user.name}</td>
                                        ...
                                        <td className="py-3 px-4 text-right">
                                            <span className="font-semibold text-slate-900">{sale.total.toLocaleString()} GNF</span>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>

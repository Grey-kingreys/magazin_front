        ...
        <div className="space-y-6">
            ...
            <div className="flex items-start justify-between">
                <div>
                    <div className="flex items-center gap-3 mb-2">
                        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-4 py-2 rounded-xl shadow-lg">
                            <div className="flex items-center gap-2">
                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                                </svg>
                                <span className="font-bold">{user.store.name}</span>
                            </div>
                        </div>
                        <span className="text-slate-500">📍 {user.store.city}</span>
                    </div>
                    <h2 className="text-2xl font-bold text-slate-800">Tableau de bord Magasin</h2>
                    <p className="text-slate-600 mt-1">Vue opérationnelle et gestion quotidienne</p>
                </div>
                <button
                    onClick={fetchData}
                    ...
                    <div className={theme.card.body}>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {cashRegisterData.openRegisters.map((register: any) => (
                                <div key={register.id} className="p-4 border-2 border-indigo-200 bg-indigo-50 rounded-lg">
                                    <div className="flex items-start justify-between mb-3">
                                        <div>
                                            <p className="font-semibold text-slate-800">{register.user.name}</p>
                                    ...
                                    <div className="space-y-2">
                                        <div className="flex justify-between text-sm">
                                            ...
                                            <span className="font-semibold text-slate-900">{register.openingAmount.toLocaleString()} GNF</span>
                                        </div>
                                        <div className="flex justify-between text-sm">
                                        ...
                                        <div className="flex justify-between text-sm">
                                            ...
                                            <span className="font-bold text-green-600">{register.currentRevenue.toLocaleString()} GNF</span>
                                        </div>
                                        <div className="pt-2 border-t border-indigo-200">
                                            <div className="flex justify-between text-sm">
                                                <span className="text-slate-600">Espèces attendues:</span>
                                                <span className="font-bold text-slate-900">{register.expectedCash.toLocaleString()} GNF</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            {data.topProducts.slice(0, 8).map((item: any, index: number) => (
                                <div key={item.product.id} className="flex items-center justify-between p-2 hover:bg-slate-50 rounded-lg transition">
                                    ...
                                    <div className="text-right">
                                        <p className="font-semibold text-slate-900 text-sm">{item.quantitySold} vendus</p>
                ...
                <div className={theme.card.base}>
                    <div className={theme.card.header}>
                        <div className="flex items-center justify-between">
                            <span>⚠️ Produits à réapprovisionner</span>
                    ...
                    <div className={theme.card.body}>
                        {data.lowStockProducts.length === 0 ? (
                            <div className="text-center py-8">
                                ...
                                <p className="text-slate-600 font-medium">Stock optimal !</p>
                                ...
                                {data.lowStockProducts.map((item: any) => (
                                    <div key={item.product.id} className="flex items-center justify-between p-3 bg-orange-50 border border-orange-200 rounded-lg">
                                        <div className="flex-1">
                                            <p className="font-medium text-slate-800 text-sm">{item.product.name}</p>
                                            <p className="text-xs text-slate-600 mt-0.5">{item.product.category.name}</p>
                                        </div>
                                        <div className="text-right">
            ...
            <div className={theme.card.base}>
                ...
                <div className={theme.card.body}>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <button className={`${theme.button.base} ${theme.button.primary} py-3 justify-center`}>
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                            </svg>
                            Nouvelle vente
                        </button>
                        <button className={`${theme.button.base} ${theme.button.secondary} py-3 justify-center`}>
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
